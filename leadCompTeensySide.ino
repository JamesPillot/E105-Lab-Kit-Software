...
//Lead-Lag controller variables
double pole = 0; //for pole location on root locus
double zero = 0; //for zero location on root locus
//Lead-Lag controller gains
double alpha = 0;
double T = 0;
double K = 0;
...
//for output
int motor_output = 0;
double lead_comp_output = 0;
double last_lead_comp_output = 0;
...
void loop()
{
  ...
  if(incoming_message[0] != '\0') //only parse message if there is something to read
  {
    ...
    else
    {
      ...
      K = atof(strtok(NULL, " "));
      pole = atof(strtok(NULL, " "));
      zero = atof(strtok(NULL, " "));
    ...
    //determine constants from pole and zero location
    T = (1.0 / zero);
    alpha = (1.0 / (pole * T));
    
    //lead-comp controller
    lead_comp_output = ((K*alpha*T*theta_error) + (K*alpha*theta_error) + (alpha*T*last_lead_comp_output)) / (1.0 + (alpha*T));
    last_lead_comp_output = lead_comp_output;
    
    /* Output Generation */
    motor_output = (int)lead_comp_output; //Assign output of selected controller to motor
    if(!new_step) //if we are not generating new step, end control loop
    {
      motor_output = 0;
      lead_comp_output = 0;
      last_lead_comp_output = 0;
    }
